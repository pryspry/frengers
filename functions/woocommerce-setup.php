...
if ( ! function_exists( 'frengers_woocommerce_setup' ) ) :
  function frengers_woocommerce_setup() {
    add_theme_support( 'woocommerce' );
  }
endif;
add_action( 'after_setup_theme', 'frengers_woocommerce_setup' );
/*
Set Default Thumbnail Sizes for WooCommerce Product Pages, on Theme Activation
*/
global $pagenow;
if ( is_admin() && isset( $_GET['activated'] ) && $pagenow == 'themes.php' ) add_action( 'init', 'frengers_woocommerce_image_dimensions', 1 );
/*
Define image sizes
*/
function frengers_woocommerce_image_dimensions() {
  $catalog = array(
		'width' 	=> '350',	// px
		'height'	=> '453',	// px
		'crop'		=> 1 		// true
...
add_action('woocommerce_before_main_content', 'frengers_wrapper_start', 10);
add_action('woocommerce_after_main_content', 'frengers_wrapper_end', 10);
...
function frengers_commerce_switch_buttons( $button ){
...
*/
function frengers_woomenucart($menu, $args) {
	...
	ob_start();
		global $woocommerce;
		$viewing_cart = __('View your shopping cart', 'frengers');
		$start_shopping = __('Start shopping', 'frengers');
		$cart_url = $woocommerce->cart->get_cart_url();
		$shop_page_url = get_permalink( woocommerce_get_page_id( 'shop' ) );
		$cart_contents_count = $woocommerce->cart->cart_contents_count;
		$cart_contents = sprintf(_n('%d item', '%d items', $cart_contents_count, 'frengers'), $cart_contents_count);
...
add_filter('wp_nav_menu_items','frengers_woomenucart', 10, 2);
