...
<?php if(have_posts()): while(have_posts()): the_post();?>
    ...
    <article role="article" id="post_<?php the_ID()?>">
        <header>
            ...
            <h4>
              <em>
                <span class="text-muted author"><?php _e('By', 'frengers'); echo " "; the_author() ?>,</span>
        ...
        <section>
            ...
            <?php the_content( __( '&hellip; ' . __('Continue reading', 'frengers' ) . ' <i class="glyphicon glyphicon-arrow-right"></i>', 'frengers' ) ); ?>
        </section>
        <footer>
            <p class="text-muted" style="margin-bottom: 20px;">
                <i class="glyphicon glyphicon-folder-open"></i>&nbsp; <?php _e('Category', 'frengers'); ?>: <?php the_category(', ') ?><br/>
                <i class="glyphicon glyphicon-comment"></i>&nbsp; <?php _e('Comments', 'frengers'); ?>: <?php comments_popup_link(__('None', 'frengers'), '1', '%'); ?>
            </p>
        </footer>
    </article>
...
<?php if ( function_exists('frengers_pagination') ) { frengers_pagination(); } else if ( is_paged() ) { ?>
  <ul class="pagination">
    <li class="older"><?php next_posts_link('<i class="glyphicon glyphicon-arrow-left"></i> ' . __('Previous', 'frengers')) ?></li>
    <li class="newer"><?php previous_posts_link(__('Next', 'frengers') . ' <i class="glyphicon glyphicon-arrow-right"></i>') ?></li>
  </ul>
<?php } ?>
